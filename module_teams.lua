...
--    local text_w = res.font:width(args.text, text_size)
    ...
    if args.team.current_standing.swiss_opponent_score_avg then
        a.add(anims.my_moving_font(S,E, 1000, y, "Avg Opponent Swiss Score: ".. args.team.current_standing.swiss_opponent_score_avg, text_size, 1,1,1,1))
    end
    y = y + text_size + 10
    ...
    a.add(anims.my_moving_font(S, E, 1000, y, "Game Score Diff", text_size, 1,1,1,1));
    a.add(anims.my_moving_font(S, E, 1200, y, "Swiss Score Diff", text_size, 1,1,1,1));
    S = S + 0.1
    y = y + text_size + 20
...
--        end
        if game.score_diff then
            a.add(anims.my_moving_font(S, E, 1000, y, game.score_diff, text_size, 1,1,1,1));
        end
        if game.swiss_score_diff then
            a.add(anims.my_moving_font(S, E, 1200, y, game.swiss_score_diff, text_size, 1,1,1,1));
        end
        S = S + 0.1
        y = y + text_size + 20
    end
